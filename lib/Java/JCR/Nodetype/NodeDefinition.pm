package Java::JCR::Nodetype::NodeDefinition;
...
study_classes(['javax.jcr.nodetype.NodeDefinition'], 'Java::JCR');
...
sub allows_same_name_siblings {
    my $self = shift;
    my @args = Java::JCR::Base::_process_args(@_);

    my $result = eval { $self->{obj}->allowsSameNameSiblings(@args) };
    if ($@) { my $e = Java::JCR::Exception->new($@); croak $e }
...
sub get_required_primary_types {
    my $self = shift;
    my @args = Java::JCR::Base::_process_args(@_);

    my $result = eval { $self->{obj}->getRequiredPrimaryTypes(@args) };
    if ($@) { my $e = Java::JCR::Exception->new($@); croak $e }
...
sub get_default_primary_type {
    my $self = shift;
    my @args = Java::JCR::Base::_process_args(@_);

    my $result = eval { $self->{obj}->getDefaultPrimaryType(@args) };
    if ($@) { my $e = Java::JCR::Exception->new($@); croak $e }
...
For full documentation of what this class does, see the Java API documentation: L<http://www.day.com/maven/jsr170/javadocs/jcr-1.0/javax/jcr/nodetype/NodeDefinition.html>
...
The package to use is L<Java::JCR::Nodetype::NodeDefinition>, rather than I<javax.jcr.nodetype.NodeDefinition>.
...
L<Java::JCR>, L<http://www.day.com/maven/jsr170/javadocs/jcr-1.0/javax/jcr/nodetype/NodeDefinition.html>
